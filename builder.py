...
###################################################################################################################################
############################################ Clean@Builder: v5 ####################################################################
############################################ Clean@Author: https://github.com/EMRE13333 ###########################################
############################################ Clean@Code: For  ZENİTH++ #########################################################
###################################################################################################################################
...
##BUILDER USING
    ...
    def compile(self, filename):
        ...
        if self.hide.get() == "yes":
            if self.icb.get() == "yes":
                os.system(f'python -m PyInstaller --onefile --version-file=./zenith_sss/version/version.txt --hiddenimport=pypiwin32 --uac-admin --noconsole --upx-dir=./zenith_sss/upx -i {icon} --distpath ./ .\\{filename}.py')
            if self.icb.get() == "no":
                os.system(f'python -m PyInstaller --onefile --version-file=./zenith_sss/version/version.txt --hiddenimport=pypiwin32 --noconsole --upx-dir=./zenith_sss/upx -i {icon} --distpath ./ .\\{filename}.py')
        if self.hide.get() == "no":
            if self.icb.get() == "yes":
                os.system(f'python -m PyInstaller --hiddenimport=pypiwin32 --version-file=./zenith_sss/version/version.txt --uac-admin --onefile --upx-dir=./zenith_sss/upx -i {icon} --distpath ./ .\\{filename}.py')
            if self.icb.get() == "no":
                os.system(f'python -m PyInstaller --hiddenimport=pypiwin32 --version-file=./zenith_sss/version/version.txt --onefile --upx-dir=./zenith_sss/upx -i {icon} --distpath ./ .\\{filename}.py')
